...
// StringToTitle converts a string to title case with proper word boundaries.
// It handles:
// - Empty strings
// - Single words (lowercase, uppercase, mixed case)
// - Multiple words with different separators (spaces, underscores, hyphens)
// - Camel case strings
// - Numbers mixed with letters
// - Special cases like "JSONData" and "APIResponse"
func StringToTitle(s string) string {
    if s == "" {
        return s
    }

    // Check if it's a single word without separators and not camel case
    if !strings.ContainsAny(s, " _-") && !isCamelCase(s) && !containsNumber(s) {
        // It's a single word (possibly with mixed case like "hElLo")
    ...
    // Helper function to process and write a word
    writeWord := func(begin, end int) {
        if begin == end {
            return
        }
        word := string(runes[begin:end])
        if result.Len() > 0 {
            result.WriteRune(' ')
        }
    ...
    if (inWord || inNumber) && start < len(runes) {
        writeWord(start, len(runes))
    }

    return result.String()
}

// shouldSplit determines if we should split the word at the current position
func shouldSplit(prev, curr, next rune) bool {
    if prev == 0 { // First character
        return false
    }

    // Split when going from lower to upper
    if isLower(prev) && isUpper(curr) {
        return true
    }

    // Split when going from upper to lower, but only if the previous
    // character wasn't the start of an acronym
    if isUpper(prev) && isUpper(curr) && next != 0 && isLower(next) {
        return true
    }

    // Split when going from letter to digit
    if (isLower(prev) || isUpper(prev)) && isDigit(curr) {
        return true
    }

    // Split when going from digit to letter
    if isDigit(prev) && (isLower(curr) || isUpper(curr)) {
        return true
    }
...
// containsNumber checks if the string contains any digits
...
// isCamelCase checks if a string without separators is in camel case format
// Returns true if the string has a pattern that indicates real camel case
// (not just alternating case like hElLo)
func isCamelCase(s string) bool {
    runes := []rune(s)
    
    // Count transitions to detect alternating pattern
    lowerToUpperCount := 0
    upperToLowerCount := 0
    
    for i := 0; i < len(runes)-1; i++ {
        if isLower(runes[i]) && isUpper(runes[i+1]) {
            lowerToUpperCount++
        } else if isUpper(runes[i]) && isLower(runes[i+1]) {
            upperToLowerCount++
        }
    }
    
    // If it's alternating case (like hElLo), counts will be similar and high
    if lowerToUpperCount > 1 && upperToLowerCount > 1 && 
       lowerToUpperCount == upperToLowerCount {
        return false
    }
    
    // Check for real camel case patterns
    for i := 0; i < len(runes)-1; i++ {
        if isLower(runes[i]) && isUpper(runes[i+1]) {
            // Check if there are at least 2 consecutive lowercase before uppercase
            if i > 0 && isLower(runes[i-1]) {
                return true
            }
            // Check if next character after uppercase is lowercase (real camel case)
            if i+2 < len(runes) && isLower(runes[i+2]) {
                // But not if it's followed by another uppercase (alternating)
                if i+3 >= len(runes) || !isUpper(runes[i+3]) {
                    return true
                }
            }
        }
    ...
    // Also check for patterns like "APIResponse" or "JSONData"
...
// processWord converts a word to title case format
func processWord(word string) string {
    ...
    // Special handling for single character
    if len(runes) == 1 {
        return string(toUpper(runes[0]))
    }

    // Convert first character to uppercase and rest to lowercase
    result := make([]rune, len(runes))
    result[0] = toUpper(runes[0])
    for i := 1; i < len(runes); i++ {
        result[i] = toLower(runes[i])
    }
